...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt, ):
    ...
    loss_giou = ((1 - torch.diag(segment_ops.segment_iou(
        ...
        segment_ops.segment_cw_to_t1t2(tgt_boxes[..., 2:])))) +
                 (1 - torch.diag(segment_ops.segment_iou(
                     src_boxes[..., :2], tgt_boxes[..., :2])))) / 2
    losses['tgt_loss_giou'] = loss_giou.sum() / num_tgt
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    if len(tgt_labels_) == 0:
        return {'tgt_loss_ce': torch.as_tensor(0.).to('cuda'),
    ...
    # loss_ce = F.cross_entropy(src_logits, tgt_labels)
